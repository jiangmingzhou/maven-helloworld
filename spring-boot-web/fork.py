...
def fork(project, directory):
    ...
    s_log4j = os.path.join(dest_dir, "src", "main", "resources", "log4j2.xml")
    d_log4j = os.path.join(dest_dir, "src", "main", "resources", "log4j2-new.xml")
    with open(s_log4j, 'r+') as fds:
        s_lines = fds.readlines()
        with open(d_log4j, 'w+') as fdd:
            for s_line in s_lines:
                # replace log file name
                if "fileName=\"logs/helloworld.log\"" in s_line:
                    d_line = s_line.replace("helloworld", project)
                    fdd.write(d_line)
                else:
                    fdd.write(s_line)
